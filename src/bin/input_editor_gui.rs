...
impl InputEditorApp {
    ...
    fn save_csv(&self, path: &PathBuf) -> Result<(), String> {
        let mut content = String::from("duration,direction,btn_a1,btn_a2,btn_b,btn_w,btn_start\n");
        ...
        std::thread::spawn(move || {
            let result = match backend_type {
                BackendType::Gpu => Self::extract_from_video_impl_thread::<WgpuBackend>(
                    video_path, model_path, tx.clone(), cancel_flag
                ),
            ...
            // 結果を送信
            match result {
                Ok(records) => { let _ = tx.send(ExtractionResult::Complete(records)); },
                Err(e) => { let _ = tx.send(ExtractionResult::Error(e)); },
            }
        });
    ...
    fn extract_from_video_impl_thread<B: burn::tensor::backend::Backend>(
        ...
        fs::create_dir_all(&temp_frames_dir).map_err(|e| format!("ディレクトリ作成エラー: {}", e))?;
        ...
        for (frame_idx, frame_path) in frame_paths.iter().enumerate() {
            ...
            let state = Self::extract_state_from_frame_static::<B>(frame_path, &model, &device, &temp_dir)
    ...
    fn extract_state_from_frame_static<B: burn::tensor::backend::Backend>(
        ...
        for (icon_idx, icon_img) in icons.iter().enumerate() {
            ...
            let tensor = Tensor::<B, 1>::from_floats(image_data.as_slice(), device)
                .reshape([1, 3, 48, 48]);
        ...
        egui::TopBottomPanel::top("menu_bar").show(ctx, |ui| {
            egui::menu::bar(ui, |ui| {
                ui.menu_button("ファイル", |ui| {
                    ...
                    if ui.button("開く (CSV/動画)").clicked() {
                            ...
                            .pick_file()
                        {
                            ...
                            match ext.to_lowercase().as_str() {
                                ...
                                _ => {
                                    self.status_message = "対応していないファイル形式です".to_string();
                                }
                            }
                        }
                        ui.close_menu();
                    ...
                    if ui.button("保存").clicked() {
                        if let Some(ref path) = self.current_file {
                            ...
                            self.status_message = "保存先を指定してください（名前を付けて保存）".to_string();
                        }
                        ui.close_menu();
                    }
                ...
                ui.menu_button("設定", |ui| {
                    ui.label("推論バックエンド:");
                    ui.radio_value(&mut self.backend_type, BackendType::Gpu, "GPU (WGPU)");
                    ui.radio_value(&mut self.backend_type, BackendType::Cpu, "CPU (NdArray)");
                    ...
                    ui.horizontal(|ui| {
                        ui.label("動画長さ上限 (秒):");
                        ui.add(egui::DragValue::new(&mut self.max_video_duration_secs)
                            .speed(1.0)
                            .range(10..=600));
                    });
                    ui.label(format!("現在: {}秒 ({}:{:02})", 
                ...
                egui::Grid::new("input_grid")
                    ...
                    .show(ui, |ui| {
                        ...
                        for (i, record) in self.records.iter_mut().enumerate() {
                            ...
                            ui.add(egui::DragValue::new(&mut record.duration).range(1..=u32::MAX));
                            ...
                            ui.add_enabled_ui(can_delete, |ui| {
                                if ui.button("❌").on_hover_text(
                                    if can_delete { "この行を削除" } else { "最低1行必要" }
                                ).clicked() {
