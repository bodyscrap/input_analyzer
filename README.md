...
### 入力インジケータ解析
- フレームから最新入力行(最下行)の領域を自動抽出
- 座標 (204, 902) から 336x48 ピクセルの領域を解析
- 各セル画像に分割（継続フレーム数 + 方向キー + ボタン5種）
- 継続フレーム数は画像から読み取らず、連続する同一入力を独自にカウント
- カスタマイズ可能な領域設定

### 入力アイコン自動認識
- 機械学習モデル(Burn)による入力アイコン認識
- 14種類の入力タイプに対応（方向9種類、ボタン5種類、空）
- 信頼度スコア付き認識結果
- GPU/CPU両対応
...
これにより `config.json` が生成され、以下の設定が保存されます：
- 計算デバイスの種類（GPU/CPU）
- モデルファイルのパス
- 学習パラメータ（エポック数、バッチサイズなど）

設定ファイルは手動で編集可能です。詳細は [CONFIG.md](CONFIG.md) を参照してください。
